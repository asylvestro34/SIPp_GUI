...
class SIPpUI(QWidget):
    def __init__(self):
        super().__init__()
        self.sipp_path_file = "sipp_path.json"
        self.sipp_executable = self.load_sipp_path()
        self.initUI()
    ...
    def initUI(self):
        ...
        self.sippPath = QLineEdit(self.sipp_executable)
    ...
    def load_sipp_path(self):
        if os.path.exists(self.sipp_path_file):
            with open(self.sipp_path_file, 'r') as file:
                return json.load(file).get("sipp_executable", "")
        return ""

    def save_sipp_path(self, path):
        with open(self.sipp_path_file, 'w') as file:
            json.dump({"sipp_executable": path}, file)
    ...
    def browseSIPP(self):
        ...
        if fileName:
            ...
            self.save_sipp_path(fileName)  # Save the path
    ...
    def runSIPP(self):
        ...
        qos_value = self.qosDropdown.currentText().split(" - ")[1].lower()
